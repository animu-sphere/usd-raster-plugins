...
| `usdGeoTiff` | `libs/usd-geotiff` | plain CMake/OpenStrata static library | implemented, not connected | GeoTIFF metadata reading behind `GeoTiffReader`: header and IFD traversal, strip/tile layout, sample format, BigTIFF, GeoTIFF keys, `ModelPixelScale` / `ModelTiepoint` / `ModelTransformation`, CRS basics, NoData, and band metadata. Pixel reads and libtiff integration remain planned. |
...
| `raster-geotiff` | `plugins/raster-geotiff` | OpenStrata plugin bundle (`usd-fileformat`) | planned | OpenUSD `SdfFileFormat` adapter for `.tif` / `.tiff`: plugin registration, argument normalization, `ArAsset` adaptation, `GeoTiffReader` construction, and authoring through the shared library. Owns its `GTIFxxx` diagnostic codes. |
