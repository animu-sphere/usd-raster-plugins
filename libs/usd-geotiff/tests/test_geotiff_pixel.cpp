...
int main() {
    ...
    Check(tileReader.ReadTile({0, 1, 1}, options, &grid, &diagnostics),
          "native TIFF tile");
    ...
    diagnostics.Clear();
    Check(!tileReader.ReadTile({1, 0, 0}, options, &grid, &diagnostics) &&
              HasCode(diagnostics, usdgeo::DiagnosticCode::UnsupportedOverviewLevel),
          "native tile rejects non-zero levels");
    diagnostics.Clear();
    Check(!tileReader.ReadTile({0, 2, 0}, options, &grid, &diagnostics) &&
              HasCode(diagnostics, usdgeo::DiagnosticCode::WindowOutOfBounds),
    ...
    Check(stripTileReader.ReadTile({0, 0, 1}, options, &grid, &diagnostics),
          "native TIFF strip");
